...
const ListBulletIcon = (props: React.SVGProps<SVGSVGElement>) => (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" {...props}>
        <path fillRule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12zm11.32-2.43a.75.75 0 01.04 1.06l-3 3a.75.75 0 01-1.06 0l-1.5-1.5a.75.75 0 111.06-1.06L10 11.69l2.47-2.47a.75.75 0 011.06-.04z" clipRule="evenodd" />
    </svg>
);
...
export default function DashboardClientComponent({ stats }: Props) {
  ...
  const summary = useMemo(() => {
    return stats.reduce((acc, vendedor) => {
      ...
      return acc;
    }, { total: 0, atendidos: 0, naoAtendidos: 0, pendentes: 0 });
  }, [stats]);

  const chartData = [
    { name: 'Vai Atender', value: summary.atendidos, color: '#10B981' }, // Verde
    { name: 'Não Atender', value: summary.naoAtendidos, color: '#EF4444' }, // Vermelho
    { name: 'Pendentes', value: summary.pendentes, color: '#F59E0B' }, // Amarelo
  ];
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
        <div className="bg-white p-5 rounded-xl shadow-lg flex items-center space-x-4">
            <div className="bg-indigo-100 p-3 rounded-full"><ListBulletIcon className="h-6 w-6 text-indigo-600" /></div>
            <div>
        ...
        <div className="bg-white p-5 rounded-xl shadow-lg flex items-center space-x-4">
            <div className="bg-green-100 p-3 rounded-full"><CheckCircleIcon className="h-6 w-6 text-green-600" /></div>
            <div>
                <p className="text-sm font-medium text-gray-500">Respondidos</p>
                <p className="text-2xl font-bold text-gray-900">{summary.atendidos + summary.naoAtendidos}</p>
            </div>
        </div>
        <div className="bg-white p-5 rounded-xl shadow-lg flex items-center space-x-4">
            <div className="bg-yellow-100 p-3 rounded-full"><ClockIcon className="h-6 w-6 text-yellow-600" /></div>
            <div>
                <p className="text-sm font-medium text-gray-500">Pendentes</p>
                <p className="text-2xl font-bold text-gray-900">{summary.pendentes}</p>
            </div>
        </div>
      </div>

      {/* --- Seção 2: Gráfico e Tabela --- */}
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-5">
        ...
        <div className="lg:col-span-3 bg-white p-6 rounded-xl shadow-lg">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4">
            <h2 className="text-lg font-semibold text-gray-900 mb-2 sm:mb-0">Resumo por Vendedor</h2>
            <button
              onClick={handleExport}
              disabled={isExporting}
              className="px-4 py-2 bg-indigo-600 text-white text-sm font-semibold rounded-lg hover:bg-indigo-700 disabled:bg-gray-400 transition-colors"
            >
